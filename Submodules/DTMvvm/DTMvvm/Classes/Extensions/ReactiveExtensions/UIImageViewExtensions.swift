...
public struct NetworkImage {
    ...
    public private(set) var completion: ((DataResponse<UIImage, AFIError>) -> Void)? = nil
    
    public init(withURL url: URL? = nil, placeholder: UIImage? = nil, completion: ((DataResponse<UIImage, AFIError>) -> Void)? = nil) {
        self.url = url
...
public extension Reactive where Base: UIImageView {
    ...
    func networkImage(_ imageTransition: UIImageView.ImageTransition = .crossDissolve(0.25), completion: ((DataResponse<UIImage, AFIError>) -> Void)? = nil) -> Binder<NetworkImage> {
